...
func TestPoSSelectionDeterministic(t *testing.T) {
    ...
    if err != nil { t.Fatal(err) }
    if v1 != v2 || p1 != p2 { t.Fatal("selection must be deterministic for same height and prev hash") }
}
...
func TestDoubleSpendPreventionWithUTXO(t *testing.T) {
    ...
    utxos := bc.ListUTXO()
    var txid string; var idx int; var amount int64; found := false
    for _, e := range utxos { if e.Output.To == "a" { txid = e.TxID; idx = e.Index; amount = e.Output.Amount; found = true; break } }
    if !found { t.Fatal("no utxo for a") }
    ...
    if err := bc.SubmitTransaction(tx1); err != nil { t.Fatal(err) }
    ...
    if err := bc.SubmitTransaction(tx2); err == nil { t.Fatal("expected rejection due to conflicting pending spend") }
}
...
func TestSnapshotPersistence(t *testing.T) {
    ...
    if err := bc.SubmitTransaction(tx); err != nil { t.Fatal(err) }
    if _, err := bc.ProposeBlock(100); err != nil { t.Fatal(err) }
    // Save snapshot
    dir := t.TempDir()
    path := filepath.Join(dir, "snap.json")
    if err := persist.Save(path, bc.Snapshot()); err != nil { t.Fatal(err) }
    // Load snapshot
    s, err := persist.Load(path)
    if err != nil { t.Fatal(err) }
    bc2 := chain.NewBlockchain(map[string]int64{}, map[string]uint64{})
    if err := bc2.LoadSnapshot(s); err != nil { t.Fatal(err) }
    if _, err := bc2.VerifyChain(); err != nil { t.Fatalf("loaded chain invalid: %v", err) }
}
...
func TestSignedTransactionVerification(t *testing.T) {
    ...
    utxos := bc.ListUTXO()
    var txid string; var idx int; var amount int64
    for _, e := range utxos { if e.Output.To == "a" { txid = e.TxID; idx = e.Index; amount = e.Output.Amount; break } }
    ...
    utxos2 := bc.ListUTXO()
    var txid2 string; var idx2 int; var amt2 int64
    for _, e := range utxos2 { if e.Output.To == addr { txid2 = e.TxID; idx2 = e.Index; amt2 = e.Output.Amount; break } }
    ...
    if err := bc.SubmitTransaction(tx2); err != nil { t.Fatalf("signed tx rejected: %v", err) }
}
...
func TestMultipleTransactionsDifferentUTXOsInSameBlock(t *testing.T) {
    ...
    if len(inputs) < 2 { t.Fatalf("expected at least 2 UTXOs for 'a', got %d", len(inputs)) }
    ...
    if err := bc.SubmitTransaction(tx1); err != nil { t.Fatalf("tx1 rejected: %v", err) }
    if err := bc.SubmitTransaction(tx2); err != nil { t.Fatalf("tx2 rejected: %v", err) }
    // Propose a block with both txs
    b, err := bc.ProposeBlock(100)
    if err != nil { t.Fatalf("propose failed: %v", err) }
    // Expect coinbase + 2 txs
    if got, want := len(b.Txns), 3; got != want { t.Fatalf("expected %d txs in block (including coinbase), got %d", want, got) }
    // Check balances reflect transfers: b should receive 200 (+ any prior), a should be reduced by 200
    if bc.GetBalance("b") < 200 { t.Fatalf("recipient balance < 200, got %d", bc.GetBalance("b")) }
}
