...
func NewBlockchain(genesisBalances map[string]int64, genesisStakes map[string]uint64) *Blockchain {
    ...
    for a, b := range genesisBalances {
        ...
        if b <= chunk {
            ...
            rem := b % chunk
            idx := 0
            for i := 0; i < n; i++ { st.UTXO[makeKey(txid, idx)] = types.TxOutput{To: a, Amount: chunk}; idx++ }
            if rem > 0 { st.UTXO[makeKey(txid, idx)] = types.TxOutput{To: a, Amount: rem} }
        }
    }
    ss := &StakeState{Stakes: map[string]uint64{}}
    for v, s := range genesisStakes { ss.Stakes[v] = s }
...
func (bc *Blockchain) Head() *types.Block {
    bc.mu.RLock(); defer bc.mu.RUnlock()
...
func (bc *Blockchain) SubmitTransaction(tx types.Transaction) error {
    ...
    if tx.From != "coinbase" && (tx.SigHex != "" || tx.PubKeyHex != "") {
        ...
        if isHexAddress(tx.From) {
            ...
            if addr != tx.From { return errors.New("from does not match pubkey address") }
        }
    }
    inSum := int64(0)
    for _, in := range tx.Inputs {
        utxo, ok := bc.state.UTXO[makeKey(in.TxID, in.Index)]
        if !ok { return fmt.Errorf("input not found: %s:%d", in.TxID, in.Index) }
        if utxo.To != tx.From { return errors.New("input does not belong to sender") }
        inSum += utxo.Amount
    }
    outSum := int64(0)
    for _, out := range tx.Outputs { if out.Amount <= 0 { return errors.New("output amount must be positive") }; outSum += out.Amount }
    if outSum > inSum { return errors.New("outputs exceed inputs") }
    // Check mempool duplicates by ID and prevent spending the same exact inputs twice
    reserved := map[string]struct{}{}
    for _, m := range bc.mempool {
        if m.ID == tx.ID { return errors.New("duplicate tx id in mempool") }
        for _, min := range m.Inputs { reserved[makeKey(min.TxID, min.Index)] = struct{}{} }
    }
    for _, in := range tx.Inputs { if _, exists := reserved[makeKey(in.TxID, in.Index)]; exists { return errors.New("conflicting pending spend of same input") } }
...
func selectionCore(prevHash string, height uint64, stakes map[string]uint64) (validator string, proof string, err error) {
    if len(stakes) == 0 { return "", "", errors.New("no validators") }
    total := uint64(0)
    keys := make([]string, 0, len(stakes))
    for k, v := range stakes { if v > 0 { total += v; keys = append(keys, k) } }
    if total == 0 { return "", "", errors.New("zero total stake") }
    ...
    for _, k := range keys {
        s := stakes[k]
        if s == 0 { continue }
        if pick >= cum && pick < cum+s { return k, hex.EncodeToString(seedBytes[:]), nil }
        cum += s
    }
    // Fallback
    for _, k := range keys { if stakes[k] > 0 { return k, hex.EncodeToString(seedBytes[:]), nil } }
...
func (bc *Blockchain) PoSSelection(height uint64) (validator string, proof string, err error) {
    ...
    for k, v := range bc.stake.Stakes { stakesCopy[k] = v }
    bc.mu.RUnlock()
...
func (bc *Blockchain) ProposeBlock(maxTx int) (*types.Block, error) {
    ...
    st := &State{Balances: map[string]int64{}, UTXO: map[string]types.TxOutput{}}
    for a, b := range bc.state.Balances { st.Balances[a] = b }
    for k, v := range bc.state.UTXO { st.UTXO[k] = v }

    // Sort mempool by timestamp then lexical ID for determinism
    sort.Slice(bc.mempool, func(i, j int) bool {
        if bc.mempool[i].Timestamp == bc.mempool[j].Timestamp { return bc.mempool[i].ID < bc.mempool[j].ID }
    ...
    for _, tx := range bc.mempool {
        if maxTx > 0 && applied >= maxTx { break }
    ...
    if err := bc.validateBlock(b); err != nil { return nil, err }
...
func (bc *Blockchain) pruneMempool(included []types.Transaction) {
    if len(included) == 0 || len(bc.mempool) == 0 { return }
    inc := map[string]struct{}{}
    for _, t := range included { inc[t.ID] = struct{}{} }
    out := bc.mempool[:0]
    for _, t := range bc.mempool { if _, ok := inc[t.ID]; !ok { out = append(out, t) } }
    bc.mempool = out
}

// validateBlock checks previous hash linkage and recomputes hash.
func (bc *Blockchain) validateBlock(b *types.Block) error {
    if b.Index != uint64(len(bc.blocks)) { return fmt.Errorf("wrong index: %d", b.Index) }
    if b.PrevHash != bc.blocks[len(bc.blocks)-1].Hash { return errors.New("prev hash mismatch") }
    if types.ComputeMerkleRoot(b.Txns) != b.TxRoot { return errors.New("txroot mismatch") }
    if b.ComputeHash() != b.Hash { return errors.New("block hash mismatch") }
    ...
    if expectedProof != b.StakeProof { return errors.New("invalid stake proof") }
    return nil
}
...
func (bc *Blockchain) AcceptBlock(b *types.Block) error {
    ...
    st := &State{Balances: map[string]int64{}, UTXO: map[string]types.TxOutput{}}
    for a, bal := range bc.state.Balances { st.Balances[a] = bal }
    for k, v := range bc.state.UTXO { st.UTXO[k] = v }
    ...
    if len(b.Txns) > 0 {
        ...
        if cb.From == "coinbase" && len(cb.Inputs) == 0 && len(cb.Outputs) > 0 {
            outSum := int64(0)
            for _, o := range cb.Outputs { if o.Amount <= 0 { return errors.New("invalid coinbase output") }; outSum += o.Amount }
            // Allow zero or configured reward; if BlockReward defined, enforce it if >0
            if BlockReward > 0 && outSum != BlockReward { return fmt.Errorf("coinbase reward mismatch: %d", outSum) }
    ...
    for i := startIdx; i < len(b.Txns); i++ {
        if err := bc.validateAndApplyTx(st, b.Txns[i]); err != nil { return fmt.Errorf("tx %d invalid: %w", i, err) }
    }
    ...
    if startIdx < len(b.Txns) { bc.pruneMempool(b.Txns[startIdx:]) }
    return nil
}
...
type UTXOEntry struct { TxID string; Index int; Output types.TxOutput }
...
func (bc *Blockchain) UpdateStake(addr string, amount uint64) { bc.mu.Lock(); bc.stake.Stakes[addr] = amount; bc.mu.Unlock() }
...
func (bc *Blockchain) Snapshot() *Snapshot {
    ...
    for a, s := range bc.stake.Stakes { sk.Stakes[a] = s }
...
func (bc *Blockchain) LoadSnapshot(s *Snapshot) error {
    if s == nil || len(s.Blocks) == 0 { return errors.New("invalid snapshot") }
    // Validate linkage
    for i := 1; i < len(s.Blocks); i++ {
        if s.Blocks[i].PrevHash != s.Blocks[i-1].Hash { return fmt.Errorf("snapshot link mismatch at %d", i) }
        if s.Blocks[i].ComputeHash() != s.Blocks[i].Hash { return fmt.Errorf("snapshot hash mismatch at %d", i) }
        if types.ComputeMerkleRoot(s.Blocks[i].Txns) != s.Blocks[i].TxRoot { return fmt.Errorf("snapshot txroot mismatch at %d", i) }
    }
    bc.mu.Lock(); defer bc.mu.Unlock()
    bc.blocks = s.Blocks
    bc.state = s.State
...
func binaryToUint64(b []byte) uint64 {
    var v uint64
    for i := 0; i < 8 && i < len(b); i++ { v |= uint64(b[i]) << (8 * i) }
    return v
}
