...
func (n *Node) handleTx(w http.ResponseWriter, r *http.Request) {
	...
	if err := n.bc.SubmitTransaction(tx); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (n *Node) handleBlock(w http.ResponseWriter, r *http.Request) {
	...
	if err := n.bc.AcceptBlock(&b); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
...
func (n *Node) BroadcastTx(tx types.Transaction) {
	...
	for _, p := range n.Peers() {
		http.Post("http://"+p+"/tx", "application/json", bytes.NewReader(payload))
	}
}
...
func (n *Node) BroadcastBlock(b *types.Block) {
	...
	for _, p := range n.Peers() {
		http.Post("http://"+p+"/block", "application/json", bytes.NewReader(payload))
	}
}
...
func (n *Node) Sync() error {
	...
	for _, p := range n.Peers() {
		...
		if len(snap.Blocks) > len(best.Blocks) {
			...
			if err := tmp.LoadSnapshot(&snap); err != nil {
				continue
			}
			if _, err := tmp.VerifyChain(); err != nil {
				continue
			}
			// Accept
